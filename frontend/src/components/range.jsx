...
const Ranger = () => {
  ...
  const Button = ({ value, display_name }) => {
    ...
    return (
      <button
        ...
        ${range === value
            ? ' scale-[0.95]  bg-[rgba(0,0,0,0.3)] shadow-customShadow'
            : 'scale-[1.05] shadow-customShadowLight bg-[rgba(0,0,0,0.1)]'
          }`}
  ...
  return (
    <motion.section
      {...FRAMER_FADE_RIGHT}
      className={`flex flex-row sm:m-1.5 m-1 box-content bg-purple-700 h-[14%] overflow-auto overflow-x-hidden rounded-[15px] border-[3px] font-medium text-[10px] sm:text-base sm:border-[5px] border-[rgba(0,0,0,0.1)] justify-evenly items-center`}>
